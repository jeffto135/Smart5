...
export const NotificationList: React.FC<NotificationListProps> = ({ notifications, onMarkAsRead, onMarkAllRead, onDelete, onDeleteAll, onNavigate }) => {
  ...
  const isUnread = (notif: EVNotification) => {
    return currentUid && !(notif.readBy || []).includes(currentUid);
  };
  ...
  const confirmDeleteAll = () => {
    setShowConfirm({ show: true, all: true });
  };
  ...
  return (
    <div className="space-y-4 pb-20">
      <div className="mb-8 flex justify-between items-end">
        <div>
          <h2 className="text-2xl font-mono font-bold uppercase tracking-tight">系統訊息 <span className="text-cyber-green">Messages</span></h2>
          {unreadCount > 0 && (
            <p className="text-[10px] font-mono text-cyber-green font-bold uppercase tracking-wider mt-1">
              您有 {unreadCount} 則未讀訊息
            </p>
          )}
        </div>
        <div className="flex gap-4">
          {unreadCount > 0 && (
            <button 
              ...
              className="flex items-center gap-2 text-[10px] font-mono font-bold uppercase text-white/40 hover:text-cyber-green transition-colors"
            >
              <CheckSquare size={14} />
              全部已讀
            </button>
          ...
          <button 
            ...
            className="flex items-center gap-2 text-[10px] font-mono font-bold uppercase text-red-500/50 hover:text-red-500 transition-colors"
          >
            <Trash2 size={14} />
            清除所有
          </button>
      ...
      {notifications.map((notif) => {
        ...
        return (
          ...
          >
            <CyberCard className={`relative overflow-hidden transition-all duration-300 ${
              unread ? 'border-cyber-green/50 shadow-[0_0_10px_rgba(204,255,0,0.1)]' : 'opacity-60 grayscale-[0.5]'
            } ${
              notif.type === 'success' ? 'group-hover:border-cyber-green/60' : 
              notif.type === 'reminder' ? 'group-hover:border-yellow-500/60' : 
                ...
                <div className="space-y-1 flex-1">
                  <div className="flex justify-between items-start">
                    <h4 className={`text-xs font-mono font-bold uppercase tracking-wider ${
                      notif.type === 'success' ? 'text-cyber-green' :
                      notif.type === 'reminder' ? 'text-yellow-500' :
                      'text-white/80'
                    }`}>
                      {notif.title}
                      {unread && <span className="inline-block w-1.5 h-1.5 rounded-full bg-cyber-green ml-2 animate-pulse" />}
                    </h4>
              ...
              {notif.userId === 'all' && (
                <div className="absolute top-2 right-2 flex items-center gap-1 opacity-20">
                  <div className="w-1.5 h-1.5 rounded-full bg-cyber-green shadow-[0_0_5px_#CCFF00]" />
                  <span className="text-[8px] font-mono font-bold uppercase">廣播</span>
                </div>
              )}
            </CyberCard>
          </motion.div>
      ...
      <AnimatePresence>
        {selectedNotif && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <CyberCard className="relative p-6 pt-12">
                <button 
                  onClick={() => setSelectedNotif(null)}
                ...
                <div className="flex flex-col items-center text-center space-y-6">
                  ...
                  <p className="text-sm text-white/70 leading-relaxed bg-white/5 p-4 rounded-xl w-full text-left">
      ...
      <AnimatePresence>
        {showConfirm.show && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            >
              <CyberCard className="p-6 text-center space-y-6">
